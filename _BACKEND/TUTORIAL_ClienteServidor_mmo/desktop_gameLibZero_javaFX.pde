...
void setup() {
    ...
    frameRate(fps);
    blitter = createGraphics(virtualResolutionWidth, virtualResolutionHeight, JAVA2D);
    blitter.noSmooth();
    //hint(ENABLE_KEY_REPEAT);
    /*
    // SOFTWARE VERSION NOT SUPPORT RENDER FILTER MODE..
...
class sprite {
    ...
    int type = 0;                          // type of sprite.. for collisionType() check..
    ...
    void draw() {
        ...
        if (visible) {
            ...
            blitter.noStroke();
            blitter.image(graph, xCenter, yCenter);
            //blitter.shape(s);
            blitter.popMatrix();
        }
        oldX = x;
...
sprite getProcess(String id) {
    for (int i=0; i<sprites.size(); i++) {
        if (sprites.get(i).id == Integer.parseInt(id)) {
            return sprites.get(i);
        }
    }
    return null;
