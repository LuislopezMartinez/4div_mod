...
// mantener la pantalla activa durante todo el tiempo sin bloquear el movil..
...
class sprite {
    ...
    void updateSizeBody() {
        ...
        case TYPE_BOX:
            if (size==100) {
                ((FBox)b).setWidth((graph.width*sizeX)/100);
                ((FBox)b).setHeight((graph.height*sizeY)/100);
            } else {
                ((FBox)b).setWidth((graph.width*size)/100);
                ((FBox)b).setHeight((graph.height*size)/100);
            }
            break;
        case TYPE_CIRCLE:
            if (size==100) {
                ((FCircle)b).setSize( ((graph.width*sizeX)/100 + (graph.height*sizeY)/100) / 2.0  );
            } else {
                ((FCircle)b).setSize( ((graph.width*size)/100 + (graph.height*size)/100) / 2.0 );
            }
            break;
        }
    }
...
//************************************************************
//------------------------------------------------------------
void screenDrawText(PFont fnt_, int size, String text, int cod, float x, float y, float angle, color col, float alpha) {
    blitter.pushMatrix();
    blitter.camera();
    blitter.noLights();
    blitter.hint(DISABLE_DEPTH_TEST);
...
//--------------------------------------------------------------------
void screenDrawRect(float x, float y, float ancho, float alto, int align, color col, float strokeWeight, color strokeColor) {
    blitter.pushMatrix();
    blitter.strokeWeight(strokeWeight);
    blitter.stroke(strokeColor);
    blitter.fill(col);
    blitter.rectMode(align);
    blitter.rect(x, y, ancho, alto);
    blitter.popMatrix();
}
//--------------------------------------------------------------------
void screenDrawEllipse(float x, float y, float ancho, float alto, int align, color col, float strokeWeight, color strokeColor) {
    blitter.pushMatrix();
    blitter.strokeWeight(strokeWeight);
    blitter.stroke(strokeColor);
    blitter.fill(col);
    blitter.ellipseMode(align);
    blitter.ellipse(x, y, ancho, alto);
    blitter.popMatrix();
}
//------------------------------------------------------------
...
class scroll {
    ...
    float alpha = 255;
    int shape_mode = CENTER;
    int width_;
    int height_;
    //......................................
    scroll(PImage gr) {
        this.gr = gr;
        this.width_ = gr.width;
        this.height_ = gr.height;
    }
    //......................................
    //......................................
    ...
    //......................................
    void setShapeMode(int sm) {
        this.shape_mode = sm;    // CENTER/CORNER..
    }
    //......................................
    void render() {
        ...
        s.vertex(width_, 0, (width_+offset.x)*tiling.x, 0+(offset.y)*tiling.y);
        s.vertex(width_, height_, (width_+offset.x)*tiling.x, (height_+offset.y)*tiling.y);
        s.vertex(0, height_, offset.x*tiling.x, (height_+offset.y)*tiling.y);
        s.endShape(CLOSE);
...
//------------------------------------------------------------
final void clearGraphicArea(PImage dst, int dst_offset_x, int dst_offset_y, int widthArea, int heightArea, color col) {
    dst.loadPixels();
    for (int i=0; i<dst.pixels.length; i++) {
        int x = i%dst.width;
        int y = i/dst.width;
        if (y>=dst_offset_y && y<(dst_offset_y+heightArea)) {
            if (x>=dst_offset_x && x<(dst_offset_x+widthArea)) {
                dst.pixels[i] = col;
            }
        }
    }
...
//------------------------------------------------------------
void clearGraph(PImage gr, color c) {
    gr.loadPixels();
    for (int i = 0; i < gr.pixels.length; i++) {
        gr.pixels[i] = c;
    }
...
//..................................................................................
final void soundPlay(MediaPlayer mySound, boolean repeat) {
    mySound.seekTo(0);
    mySound.start();
...
class soundPlayTimed extends sprite {
    ...
    int millis = 0;
    MediaPlayer snd;
    public soundPlayTimed(MediaPlayer snd, int millis) {
        startTime = millis();
        this.millis = millis;
        this.snd = snd;
    }
...
//--------------------------------------------------------------------
// CSV HELPER FUNCTIONS CODE------------------------------------------
//--------------------------------------------------------------------
ArrayList loadCSV(String file) {
    ArrayList<String[]> matrix = new ArrayList<String[]>();  // creo una lista de listas..
    String[] lines = loadStrings(file);                      // leo el archivo de disco en modo texto..
    for (String line : lines) {                                // desde la primera hasta la ultima linea..
        String param[] = split(line, ';');                     // separar el texto de la linea segun el delimitador indicado en split() en una lista de sub_strings..
        matrix.add(param);                                     // añadir la lista de sub_strings a la matriz..
    }
    return matrix;                                           // devolver como resultado la matriz..
}
