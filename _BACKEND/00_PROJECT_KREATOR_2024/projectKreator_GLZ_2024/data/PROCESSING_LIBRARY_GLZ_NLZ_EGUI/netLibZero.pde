...
Client createClient(String ip, int port) {
...
class Server extends Thread {
    ArrayList<Servidor>conections = new ArrayList();
    ...
    void run() {
        ...
        try {
            ...
            println("Servidor iniciado en el puerto: " + PUERTO);
            while (true) {
                ...
                Servidor cc = new Servidor(this, servicio, flujoDatosEntrada, flujoDatosSalida);  //Parametros, la conexion , y los objetos de escritura/lectura
...
class Servidor extends sprite {
    ...
    public Servidor(Server idServ, Socket servicio, DataInputStream in, DataOutputStream out) {  //Constructor
    ...
    public void frame() {  //Esto es un metodo, que es lo que correra cada hilo de nustro servidor
        ...
        while ( av ) {
            ...
            String[] buffer = split(data, NET_TOK_DATA);     // trocear lo recibido del socket..
            for (int i=0; i<buffer.length; i++) {
                datum.append(buffer[i]);                     // inyectar los trozos al stringlist de salida..
            }
            onNetServerMessage(this, datum);                               // enviar salida a la función que la va a interpretar..
            av = available();
        }
        ...
        if(this.useTimeout){
            ...
            if (counter>idServerListener.timeout_frames) {
                this.close();
                println("[SERVER] Conection timeout.");
                onNetServerClose(this);
            }
        }
    ...
    //......................................
    void write(netMessage n) {
        write(n.buffer);
    }
    //......................................
    void write(StringList msg) {
        netMessage n = new netMessage(this);
...
class Client extends sprite {
    ...
    void frame() {
        ...
        while ( av ) {
            ...
            onNetClientMessage(this, datum);                 // enviar salida a la función que la va a interpretar..
            av = available();
        }
        ...
        if(this.useTimeout){
            if(this.connected){
                ...
                if (this.counter>this.timeout_frames) {
                    this.close();
                    this.connected = false;
                    println("[CLIENT] Conection timeout.");
                    onNetClientClose(this);
                }
            }
        }
...
class netMessage {
    ...
    //-------------CONSTRUCTOR MSG----------------------------
    //--------------------------------------------------------
    public netMessage(Servidor s) {
        socket_server = s;
    }
    public netMessage(Client c) {
        socket_client = c;
    }
    ...
    void broadcast() {
        if (socket_server!=null) {
            ...
            for (Servidor s : this.socket_server.idServerListener.conections) {
                s.write(this.buffer);
            }
        } else if (socket_client!=null) {
