...
window.setup = function () {
    setBackgroundColor(glz.WHITE);      // color de fondo de pantalla..
    ...
    switch (ST) {
        ...
        case 10:
            if (glz.mouse.left) {
                ...
                //loader[0] = new LoadImages("data/Example_00_text/images/", 0);
                //loader[1] = new LoadSounds("data/Example_00_text/sounds/", 0);
                // let lista = [];
                // lista.push("data/Example_00_text/models/yourModelFilename.ext);
...
class Game extends Process {
    constructor() {
        ...
        this.st = 0;
        this.idText;
        this.colors = [WHITE, SILVER, GRAY, BLACK, RED, MAROON, YELLOW, OLIVE, LIME, GREEN, BLUE, PINK, PURPLE, ORANGE];
        this.counter = 0;
    }
    frame() {
        switch (this.st) {
            case 0:
                this.idText = new Write(null, 22, "Texto de prueba!", LEFT, glz.WIDTH / 2, glz.HEIGHT / 2, RED, 1);
                fadeOn(1000);
                this.st = 10;
                break;
            case 10:
                this.counter = (this.counter + 1) % this.colors.length;
                this.idText.setColor(this.colors[this.counter]);
                this.idText.size = sin(radians(glz.frameCount)) * 10 + 22;
                this.idText.angle++;
                break;
        }
    }
