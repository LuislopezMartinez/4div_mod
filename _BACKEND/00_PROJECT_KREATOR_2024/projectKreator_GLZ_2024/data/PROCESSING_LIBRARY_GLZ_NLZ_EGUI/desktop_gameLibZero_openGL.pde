...
//************************************************************
//------------------------------------------------------------
final void screenDrawText(boolean clip, PFont fnt_, int size, String text, int cod, float x, float y, color col, float alpha) {
    blitter.pushMatrix();
    blitter.camera();
    blitter.noLights();
    blitter.hint(DISABLE_DEPTH_TEST);
...
boolean collisionMouse(sprite id) {
    ...
    ArrayList list = world.getBodies(mouse.x, mouse.y, true);

    if (list.contains(id.b)) {
        return true;
    } else {
        return false;
    }
...
//------------------------------------------------------------
void graphSetPixel(PImage gr, int x, int y, color c) {
    gr.loadPixels();
    int index = y*gr.width + x;
    gr.pixels[index] = c;
}
//------------------------------------------------------------
...
//------------------------------------------------------------
PVector calculateText(PFont fnt, int size, String text) {
    blitter.pushMatrix();
    if (fnt!=null) {
        blitter.textFont(fnt);
    }
    blitter.textSize(size);
    float width  = blitter.textWidth(text);
    float height = size + blitter.textAscent() + blitter.textDescent();
    blitter.popMatrix();
...
/*
    funcion collisionMouse modificada para detectar hasta colision con una lista de 10 fbody´s..
